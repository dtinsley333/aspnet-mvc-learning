<%@ Page Title="Part Set Up:Global Quality Alert File Editor" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<Quality.ViewModels.PartSetUpViewModel>"  %>
...
<asp:Content ID="indexContent" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div id="partsetups">
   ...
   <h2>Edit Quality Alert Links</h2>
     
      	<div class='pageheaders'>Global Update Search </div><br /><br />
        <%using (Html.BeginForm("PartSetUpGlobalQualityAlertFile1Update", "PartSetUp"))
          { %>
          ...
          <table class="clean-table" width="40%" >
            ...
            <tr><td>Search by Quality Alert File #1 Name:</td><td><input type="text" id="searchtxt" onkeyup="searchSel()"/></td></tr>
            <tr><td>Quality Alert File #1 Path:</td><td><%= Html.DropDownListFor(a => a.PartSetUp.QualityAlertFile, Model.QualityAlertFile1PathSelectList, "Search by Quality Alert #1  File Name", new { @id = "ddlQualityAlertFiles" })%></td></tr>
...
Cancel
    ...
    <%using (Html.BeginForm("GlobalQualityAlertFileEdits", "PartSetUp"))
      {
  ...
  <div class='pageheaders'>Select a Quality Alert File#1(PDF)</div>           
...
<table bgcolor="#EDEEF7" >
    ...
    <tr><td class="style1" bgcolor="#EDEEF7" >Quality Alert File #1:</td></tr>
   <tr><td>Quality Alert #1 Effective Start Date:<%=Html.TextBoxFor(model => model.PartSetUp.QualityAlertStartDte, new { @id = "startdate1" })%>&nbsp;(Date the Quality Alert is to  start printing with the Travel Card.)</td></tr>
    <tr><td>Quality Alert #1 Effective End Date:<%=Html.TextBoxFor(model => model.PartSetUp.QualityAlertEndDte, new { @id = "enddate1" })%>&nbsp;(Date the Quality Alert file is to stop printing with the Travel Card. Leaving blank means no end.)</td></tr>

      <tr> <td><label for="file">Select File (<img src="<%=pdfImagegfilepath %> "/> PDF only):</label>
 <td> <input type="file" name="QualityAlertFilePath" id="QualityAlertFilePath" accept="file/pdf"/></td><td> Remove Links <%=Html.CheckBoxFor(a => a.QualityAlertFileDelete)%></td><td><span class="button-updatequalityalertFiles"></span><input type="submit"class="button" value="Save Quality Alert File Path" /> <a href="javascript:history.go(-1)">
Cancel
</a></td></tr>
