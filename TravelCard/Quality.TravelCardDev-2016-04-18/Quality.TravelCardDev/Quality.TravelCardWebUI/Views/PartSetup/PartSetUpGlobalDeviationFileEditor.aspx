...
<script>
  ...
  <script type="text/javascript">
      function searchSel() {
          ...
          var output = document.getElementById('ddlDeviation1Files').options;
...
<asp:Content ID="indexContent" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div id="partsetups">
   ...
   <h2>Edit Deviation File Links</h2>
     
      	<div class='pageheaders'>Global Update Search </div><br /><br />
        <%using (Html.BeginForm("PartSetUpGlobalDeviationFileUpdate", "PartSetUp"))
          { %>
          ...
          <table class="clean-table" width="40%" >
            ...
            <tr><td>Deviation File Path:</td><td><%= Html.DropDownListFor(a => a.PartSetUp.DeviationFile, Model.DeviationFile1PathSelectList, "Search by Deviation File Name", new { @id = "ddlDeviation1Files" })%></td></tr>
...
Cancel
    ...
    <%using (Html.BeginForm("GlobalDeviationFileEdits", "PartSetUp"))
      {
  ...
  <div class='pageheaders'>Select a Deviation File (PDF)</div>           
...
<table bgcolor="#EDEEF7" >
   ...
   <tr><td>Deviation #1 Effective Start Date:<%=Html.TextBoxFor(model => model.PartSetUp.DeviationFileStartDte, new { @id = "startdate1" })%>&nbsp;(Date the deviation file is to  start printing with the Travel Card.)</td></tr>
    <tr><td>Deviation #1 Effective End Date:<%=Html.TextBoxFor(model => model.PartSetUp.DeviationFileEndDte, new { @id = "enddate1" })%>&nbsp;(Date the deviation file is to stop printing with the Travel Card. Leaving blank means no end.)</td></tr>

      <tr> <td><label for="file">Select File (<img src="<%=pdfImagegfilepath %> "/> PDF only):</label>
 <td> <input type="file" name="DeviationFilePath" id="DeviationFilePath" accept="file/pdf"/></td><td> Remove Links <%=Html.CheckBoxFor(a => a.DeviationFileDelete)%></td><td><span class="button-updatedeviationFiles"></span><input type="submit"class="button" value="Save Deviation File Path" /> <a href="javascript:history.go(-1)">
Cancel
</a></td></tr>
