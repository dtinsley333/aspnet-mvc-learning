<%@ Page Title="Link to 2nd Deviation File" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<Quality.ViewModels.PartSetUpViewModel>"  %>
...
<br /><br /><br /><br /><br /><br /><br />
    ...
    <% using (Html.BeginForm("GetDeviationFile2", "PartSetUp"))
		{ %>
...
<table>
    <tr><td>Deviation Effective Start Date:<%=Html.TextBoxFor(model => model.PartSetUp.DeviationFile2StartDte, new { @id = "startdate" })%>&nbsp;(Date the deviation file is to  start printing with the Travel Card.)</td></tr>
    <tr><td>Deviation Effective End Date:<%=Html.TextBoxFor(model => model.PartSetUp.DeviationFile2EndDte, new { @id = "enddate" })%>&nbsp;(Date the deviation file is to stop printing with the Travel Card. Leaving blank means no end.)</td></tr>
    <tr><td class="style1" bgcolor="#EDEEF7" >Deviation File? :<label for="file">Select File (<img src="<%=pdfImagegfilepath %> "/> PDF only):</label>
    <input type="file" name="DeviationFilePath" id="DeviationFilePath" accept="file/pdf"/>(Must always reselect the document, unless you are removing it.)</td></tr>
